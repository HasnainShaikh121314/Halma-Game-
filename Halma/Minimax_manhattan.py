...
def heuristic(board, player):
    white_score = sum(manhattan_distance((i, j), target) for i, row in enumerate(board) for j, cell in enumerate(row) if cell == 'W' for target in Target_White)
    black_score = sum(manhattan_distance((i, j), target) for i, row in enumerate(board) for j, cell in enumerate(row) if cell == 'B' for target in Target_Black)
    if player == 'white':
