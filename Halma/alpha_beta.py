...
def heuristic(board, player):
    ...
    white_pieces_in_target = sum(1 for i in range(16) for j in range(16) if board[i][j] == 'W' and (i, j) in Target_White)
    black_pieces_in_target = sum(1 for i in range(16) for j in range(16) if board[i][j] == 'B' and (i, j) in Target_Black)

    white_proximity = sum(min(abs(i - ti) + abs(j - tj) for (ti, tj) in Target_White) for i in range(16) for j in range(16) if board[i][j] == 'W')
    black_proximity = sum(min(abs(i - ti) + abs(j - tj) for (ti, tj) in Target_Black) for i in range(16) for j in range(16) if board[i][j] == 'B')
